...
// ── стикеры над головой ──────────────────────────────────
// _stickerFx (js/ui.js): у каждого свой <img> в #sticker-layer. Здесь только
// где он и насколько виден: позиция каждый кадр берётся у владельца, так что
// стикер едет вместе с игроком; ушёл владелец — стикер снимается. Своё
// движение у картинки — CSS (.stk-a-*), здесь лишь появление с перелётом,
// лёгкий подъём и исчезновение.
const _STK_SIZE = 72, _STK_ABOVE = 96;
function _stkEaseBack(k) { const c = 1.9; return 1 + (c + 1) * Math.pow(k - 1, 3) + c * Math.pow(k - 1, 2); }
function _updateStickers(camX, camY) {
  const now = performance.now();
  const life = STICKER_LIFE_MS / 1000;
  const px = _STK_SIZE * ZOOM;
  let keep = 0;
  for (let i = 0; i < _stickerFx.length; i++) {
    ...
    if (age >= life || !owner || owner.x == null || isNaN(owner.x) || (owner.hp != null && owner.hp <= 0)) {
      if (s.el) s.el.remove();
      continue;
    }
    _stickerFx[keep++] = s;
    if (!s.el) continue;
    const sc = _stkEaseBack(Math.min(1, age / 0.3));
    const alpha = age > life - 0.4 ? Math.max(0, (life - age) / 0.4) : Math.min(1, age / 0.08);
    const sx = (owner.x - camX) * ZOOM;
    const sy = HEADER_H + (owner.y - _STK_ABOVE - 8 * Math.min(1, age / life) - camY) * ZOOM;
    // Масштаб — от центра 64-пиксельной коробки (transform-origin по умолчанию).
    s.el.style.transform = `translate(${(sx - 32).toFixed(1)}px,${(sy - 32).toFixed(1)}px) scale(${(sc * px / 64).toFixed(3)})`;
