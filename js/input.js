...
// Единственное, что стоит НИЖЕ хаба, а не выше: весь веер уходит вверх-влево
// (углы FAN_A_* отрицательные), и правый нижний угол под кнопкой атаки —
// единственное свободное место рядом с большим пальцем. Это показометр, а не
// кнопка: навык питомца применяется сам, нажимать нечего — поэтому он и
// меньше навыков игрока, и хит-теста у него нет.
//
// Радиус и угол подобраны так, чтобы кружок целиком помещался в отступы хаба
// (FAN_MX/FAN_MY = 64/70 от края и навбара) и не налезал на саму атаку:
// 60·cos45° ≈ 42, плюс радиус 17 — это 59 при доступных 64 и 70, а до края
// атаки (40) остаётся три пикселя. Всё через hud(), как и остальной веер.
const FAN_R_PET = hud(60);
const FAN_A_PET = 45;
