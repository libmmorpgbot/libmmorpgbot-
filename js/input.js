...
function getTargetBtnPos() {
  ...
  if (f) {
    const r = fanRect();
    // Рядом с зельем, левее и ниже: обе кнопки идут по верхнему краю веера,
    // и палец правой руки достаёт их одним движением, не уходя к джойстику.
    // Зазор между ними намеренно маленький — 6 px между ободами: это одна
    // группа, а не две разные части экрана.
    const pot = getPotionBtnPos();
    // +8, а не +26 по вертикали: на 26 нижний край кнопки заходил на дугу
    // веера, и цель читалась как ещё одно его гнездо.
    return { x: pot.x - 58, y: pot.y + 8, r: 25 };
  }
...
function getPotionBtnPos() {
  ...
  if (f) {
    const r = fanRect();
    // Выше и левее прежнего: кнопка сидела вплотную к верхнему краю веера и
    // читалась как его часть, хотя расходник к вееру отношения не имеет.
    return { x: r.x + r.w * 0.50, y: r.y - 34, r: 27 };
  }
  const sz = SKILL_SZ, gap = SKILL_GAP, r = POTION_R;
