...
//
//   #ff4     жёлтый      0.92   ← видно
//   #98e456  зелёный      0.74
//   #ff8c00  оранжевый    0.62   ← «оранжевий»
//   #5dade2  голубой      0.61
//   #4af     синий        0.59   ← «синій колір»
//
...
function _dmgLegible(color) {
  ...
  if (h.length === 6) {
    ...
    if (Number.isFinite(n)) {
      ...
      // Воспринимаемая яркость, а не среднее: глаз видит зелёный впятеро ярче
      // синего, и среднее назвало бы #4af и #ff4 почти одинаковыми.
      const lum = (0.299 * r + 0.587 * g + 0.114 * b) / 255;
      if (lum < _DMG_MIN_LUMA) {
        // Подмешать белого ровно на недостающее. Яркость смеси линейна по доле
        // белого, поэтому доля считается, а не подбирается.
        const k = (_DMG_MIN_LUMA - lum) / (1 - lum);
