...
// Никогда не выбор игрока: единственный способ получить её — джекпот
// переработки ОРУЖЕЙНОЙ легендарной руны (RUNE_REROLL_BONUS_SKILL_CHANCE,
// shared/definitions.js, 3%), и она живёт ВНУТРИ этой руны — снял руну или
// сменил оружие, player.foreignSkill приходит null (xpSync, js/network.js),
// карточка просто не рисуется, ничего не потеряно. Эта карточка живёт
// ОТДЕЛЬНО от четырёх обычных выше: свой уровень, свой кулдаун
// (FOREIGN_SKILL_KEY) — учить её нечему, только качать книгами того же
// класса+ключа, что и её собственная механика.
function _foreignSkillCardHtml() {
  ...
  return `<div class="skill-upg-card foreign-skill-card">
    ...
    ${btnHtml}
    <div class="sk-foreign-note">${t('foreignSkillTiedToRuneNote')}</div>
  </div>`;
}
...
// fifth, independent slot (player.foreignSkill, granted only by the
// legendary-rune-reroll jackpot) has an ability in it. Formula reproduces
// the FAN_TRAY_A1 constant above exactly at
...
function drawSkillButtons() {
  ...
  // The four real slots, plus — once the equipped weapon's legendary rune
  // carries one — the fifth, independent one: its own icon/cd (from the
  // borrowed ability's own definition, not player.type's), its own cooldown
  // key (FOREIGN_SKILL_KEY, never one of Q/W/E/R), locked by its OWN level
...
// ─────────────────────────────────────────────────────────
//  НАВЫК ПИТОМЦА — кружок в правом нижнем углу, под атакой
// ─────────────────────────────────────────────────────────
// Выглядит как навык игрока и читается так же: пока идёт откат — затемнён и
// с числом, как только готов — светится. Отличие одно и оно намеренное:
// нажимать нечего. Навык питомца применяется сам раз в тридцать секунд
// (Room._petSkillTick), поэтому у кружка нет ни хит-теста, ни вспышки
// нажатия — он показывает, а не принимает.
//
// Иконка берётся с НАДЕТОГО питомца, а не с petSkillId: во время действия
// бафа питомца можно снять или сменить, и кружок обязан показывать того, кто
// будет применять навык дальше, а не того, чей баф ещё висит (для бафа есть
// своя фишка в drawBuffStrip).
function drawPetSkillButton() {
  if (!player || typeof petSkillOf !== 'function') return;
  const pet = player.equipment && player.equipment.pet;
  const sk = pet ? petSkillOf(pet.id) : null;
  if (!sk) return;                       // у неэпического питомца навыка нет

  const b = getPetBtnPos();
  const cx = b.x, cy = b.y, r = b.r;
  // Остаток отката. 0 — «фаза неизвестна» (сервер ещё не сказал), и тогда
  // кружок показывается готовым: врать числом хуже, чем не показывать его.
  const left = petSkillNextAt > 0 ? Math.max(0, (petSkillNextAt - Date.now()) / 1000) : 0;
  const active = typeof petSkillTimer !== 'undefined' && petSkillTimer > 0;
  const ready = left <= 0;

  // Фон
  ctx.fillStyle = active ? 'rgba(38,58,44,0.92)' : ready ? 'rgba(30,46,66,0.92)' : 'rgba(18,26,38,0.92)';
  ctx.beginPath(); ctx.arc(cx, cy, r, 0, Math.PI * 2); ctx.fill();

  // ── иконка видна ВСЕГДА ──────────────────────────────────────────────────
  // У навыков игрока на откате иконка гасится до 0.45 и накрывается чёрным —
  // там это работает, потому что откат кончается и кнопка снова светится.
  // Здесь бы не сработало никогда: питомец применяет навык в ту же секунду,
  // в которую откат кончился, и «готового» состояния у кружка практически
  // нет. Погашенная иконка означала бы просто иконку, которую не видно.
  //
  // Поэтому откат показывается ЗАТЕНЁННЫМ СЕКТОРОМ поверх целой иконки —
  // сектор тает по часовой стрелке, как в любой другой игре, и по нему
  // видно, сколько ещё ждать, не читая число.
  const period = (typeof PET_SKILL_PERIOD_MS !== 'undefined' ? PET_SKILL_PERIOD_MS : 30000) / 1000;
  ctx.save();
  ctx.beginPath(); ctx.arc(cx, cy, r - 1.5, 0, Math.PI * 2); ctx.clip();
  const img = _getPotImg(sk.img);
  if (img && img.complete && img.naturalWidth > 0) ctx.drawImage(img, cx - r, cy - r, r * 2, r * 2);
  // Пока навык ДЕЙСТВУЕТ — сектор не рисуется вовсе. Откат в эти секунды уже
  // идёт (он начинается с применения), и затенять кружок ровно тогда, когда
  // навык работает, значит показывать «нельзя» в момент «работает». Отсчёт
  // до следующего применения вернётся сразу, как только баф кончится.
  if (!ready && !active && period > 0) {
    const frac = Math.min(1, left / period);     // сколько ОСТАЛОСЬ
    ctx.fillStyle = 'rgba(4,8,14,0.62)';
    ctx.beginPath();
    ctx.moveTo(cx, cy);
    ctx.arc(cx, cy, r, -Math.PI / 2 + Math.PI * 2 * (1 - frac), Math.PI * 1.5);
    ctx.closePath(); ctx.fill();
  }
  ctx.restore();
  ctx.globalAlpha = 1;

  // Рамка. Пока навык действует — его собственным цветом: единственное
  // состояние, которое стоит выделить, потому что оно длится десять секунд.
  ctx.lineWidth = active ? 2.5 : 1.5;
  ctx.strokeStyle = active ? (sk.color || '#8fd17a')
    : ready ? 'rgba(203,161,89,0.7)' : 'rgba(32,56,87,0.7)';
  ctx.beginPath(); ctx.arc(cx, cy, r, 0, Math.PI * 2); ctx.stroke();

  // Число. Пока навык действует — это ЕГО остаток, а не отката: в эти
  // секунды игроку важно, сколько ещё держится прибавка. Потом — отсчёт до
  // следующего применения, тем же шрифтом и по тому же правилу, что у
  // навыков игрока (десятая доля на последней секунде).
  const num = active ? petSkillTimer : left;
  if (active || !ready) {
    ctx.font = `bold ${hudF(12)}px ${_F_SKILL}`; ctx.textBaseline = 'middle'; ctx.textAlign = 'center';
    const txt = num >= 10 ? String(Math.ceil(num)) : num.toFixed(1);
    // Обводка обязательна: число ложится прямо на картинку навыка, а она
    // яркая — без неё цифры читаются только на тёмной её половине.
    ctx.lineWidth = 3; ctx.strokeStyle = 'rgba(2,5,9,0.9)';
    ctx.strokeText(txt, cx, cy);
    ctx.fillStyle = active ? (sk.color || '#8fd17a') : '#e4ecf3';
...
// bonusSkill — джекпот легендарного перебора ОРУЖЕЙНОЙ руны
// (RUNE_REROLL_BONUS_SKILL_CHANCE, shared/definitions.js): 3% шанс на
// заимствованную способность случайного класса, пишется прямо в руну и
// заменяет любую прежнюю каждым нажатием — так что либо новая объявляется,
// либо (bonusSkillLost) прежняя молча пропала бы без этого сообщения, и
// игрок решил бы, что это баг, а не цена этого конкретного переброса.
function _bonusSkillSuffix(res) {
  if (res && res.bonusSkill) {
    const bs = res.bonusSkill;
    const sk = (typeof SKILL_DEF !== 'undefined' && (SKILL_DEF[bs.cls] || []).find(s => s.key === bs.key)) || null;
    return ` — заимствованная способность: ${sk ? sk.name : bs.key}!`;
  }
  if (res && res.bonusSkillLost) return ' — заимствованная способность руны пропала';
  return '';
}

function onRuneRerolled(res) {
  if (!res || !res.after) return;
  _shopMsgOrToast(`Новый вид: ${_runeLineLabel(res.after)}${_bonusSkillSuffix(res)}`);
  if (typeof updateInvUI === 'function') updateInvUI();
  _refreshOpenRuneModal();
}
...
function onRuneRerolledAll(res) {
  ...
  _shopMsgOrToast((parts.join(', ') || 'Переработка выполнена') + _bonusSkillSuffix(res));
  if (typeof updateInvUI === 'function') updateInvUI();
  _refreshOpenRuneModal();
}
