...
Object.assign(I18N_UI, {
  ...
  // легендарной руны, RUNE_REROLL_BONUS_SKILL_CHANCE) ─────────────────────
  ...
  foreignSkillReplaceHdr: { ru: 'Заменить другой книгой', en: 'Replace with another book', uk: 'Замінити іншою книгою', es: 'Reemplazar con otro libro', tr: 'Başka bir kitapla değiştir', pt: 'Substituir por outro livro' },
  learnForeignBtnFmt: { ru: 'Изучить: {name}', en: 'Learn: {name}', uk: 'Вивчити: {name}', es: 'Aprender: {name}', tr: 'Öğren: {name}', pt: 'Aprender: {name}' },
  learnForeignSkillConfirmFmt: { ru: 'Изучить «{name}» в пятый слот? Он не займёт Q/W/E/R.{replace}', en: 'Learn "{name}" into the fifth slot? It will not occupy Q/W/E/R.{replace}', uk: 'Вивчити «{name}» у п\'ятий слот? Він не займе Q/W/E/R.{replace}', es: '¿Aprender "{name}" en la quinta ranura? No ocupará Q/W/E/R.{replace}', tr: '"{name}" beşinci yuvaya öğrenilsin mi? Q/W/E/R\'i kaplamaz.{replace}', pt: 'Aprender "{name}" no quinto slot? Ele não ocupará Q/W/E/R.{replace}' },
  learnForeignSkillReplaceNote: { ru: ' Прежняя заимствованная способность будет заменена.', en: ' The previous borrowed ability will be replaced.', uk: ' Попередня запозичена здібність буде замінена.', es: ' La habilidad prestada anterior será reemplazada.', tr: ' Önceki ödünç yetenek değiştirilecek.', pt: ' A habilidade emprestada anterior será substituída.' },
