...
// theme purity.
const CHAR_DEF = {
  // iconImg: owner-supplied artwork (images/classicon/<cls>.png), replacing
  // the hand-drawn ICON_SVG stroke glyph this held before (js/icons.js's
  // drawClassIconCtx/drawClassBadgeCtx draw these; the old vector `icon`
  // field is gone — nothing else read it once charselect.js/js/ui.js/
  // server/handlers2/social.js were switched over, see their own comments).
  lev:         { name:'Танк',         iconImg:'/images/classicon/lev.png',         color:'#9aa3ab', baseHP:260, baseAtk:2, baseDef:14, speed:130, atkRange:58,  atkSpeed:1.000, atkType:'melee' },
  deathknight: { name:'Рыцарь Смерти',iconImg:'/images/classicon/deathknight.png', color:'#7a5c99', baseHP:200, baseAtk:3, baseDef:10, speed:145, atkRange:58,  atkSpeed:1.197, atkType:'melee' },
  ranger:      { name:'Егерь',        iconImg:'/images/classicon/ranger.png',      color:'#5c7a4a', baseHP:140, baseAtk:2, baseDef:5,  speed:175, atkRange:210, atkSpeed:1.593, atkType:'ranged', projColor:'#8fbf5a' },
  mage:        { name:'Маг',          iconImg:'/images/classicon/mage.png',        color:'#5c7fbf', baseHP:110, baseAtk:4, baseDef:3,  speed:155, atkRange:180, atkSpeed:0.837, atkType:'ranged', projColor:'#66aaff' },
  warlock:     { name:'Целитель',     iconImg:'/images/classicon/warlock.png',     color:'#8a3a4a', baseHP:160, baseAtk:2, baseDef:7,  speed:148, atkRange:170, atkSpeed:1.200, atkType:'ranged', projColor:'#a855e0' },
  runefighter: { name:'Рунный боец',  iconImg:'/images/classicon/runefighter.png', color:'#a86b3f', baseHP:230, baseAtk:3, baseDef:8,  speed:160, atkRange:58,  atkSpeed:1.100, atkType:'melee' },
  assassin:    { name:'Ассасин',      iconImg:'/images/classicon/assassin.png',    color:'#2e2438', baseHP:95,  baseAtk:4, baseDef:2,  speed:165, atkRange:56,  atkSpeed:1.350, atkType:'melee' },
};
...
//
// Ставка при этом НЕ делится. Каждый вид роняет свой пул с той же полной
// вероятностью — меняется только то, ЧТО именно выпадет, а не как часто.
...
function _splitAcrossSpecies(ids, species, offset = 0) {
  const out = {};
  species.forEach(sp => { out[sp] = []; });
  ids.forEach((id, i) => { out[species[(i + offset) % species.length]].push(id); });
  return out;
}

// Which advanced-skill books each Фарм-зона species can drop. The full
// 20-book pool (_ADV_SKILL_BOOK_SRC, 5 classes × Q/W/E/R) split round-robin
...
const FARM_SPECIES_BOOKS = _splitAcrossSpecies(
  _ADV_SKILL_BOOK_SRC.map(([cls, key]) => `book_adv_${cls}_${key}`), FARM_SPECIES);
...
const MAT_UPGRADE_RECIPES = [
  ...
  // Руда. Десять к одному, теперь БЕЗ потерь (было 50% — владелец перенёс
  // случайность в сам дроп руды, см. ORE_DROP_CHANCE, и здесь она больше не
  // нужна: терять уже добытую руду на полпути к следующему ярусу поверх
  // 5%-го дропа было бы двойным штрафом). Своего кода у лесенки нет: и
  // переплавка (upgradeMat, repos/craft.js), и вкладка «Материалы» у
  // кузнеца читают ЭТУ таблицу, так что четыре строки ниже — весь рецепт.
  { from:'ore_common',   to:'ore_uncommon',  count:10, chance:1.0 },
...
// таким правилом. Было 70% по первому указанию владельца; снижено до 5% по
// второму — вместе с тем, что переплавка (лесенка ниже) стала стопроцентной,
...
// ── Tournament ("Турнир", 16-player double elimination) ─────────────────────
// Registration opens Thursday and Sunday at 23:00 Moscow time and stays open
// until either 16 people have signed up (the bracket starts immediately,
...
// longer window because 16 sign-ups is a higher bar than 6.
...
const TOURNAMENT_ROUND_GAP_MS = 30 * 1000;
...
//
// Забрать можно один раз на аккаунт: флаг player_progress.mail_bonus2_claimed,
...
// награда.
//
// Своя колонка, а не переиспользование mail_bonus_claimed (миграция 019, тот
// же приём, что и у неё самой против starter_bonus_claimed): это новый набор
// наград текущего письма, и аккаунт, забравший старое письмо, старым письмом
// сыт не считается — заново видит кнопку и может забрать новое.
//
// Действует, пока идёт 3-й сезон: claimMailBonus (server/db/repos/shop.js)
// проверяет seasonActive() тем же способом, что и остальные сезонные
// награды, и после SEASON_END_AT кнопка выдаёт отказ, даже если флаг ещё
// не поднят.
//
...
const MAIL_BONUS = {
  ...
  // то есть «пять банок бафов всех видов» это 5 × 6. Та же мера, что и
