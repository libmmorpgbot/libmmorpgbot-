...
module.exports = function registerWorld(s, safeOn, deps) {
  ...
  function rollLoot(result) {
    ...
    // поэтому и бросается здесь, над таблицами, а не внутри каждой из
    // четырёх. Иначе «со всех» означало бы четыре одинаковых куска кода, и
    // первый же новый режим тихо остался бы без руды.
    //
    // Порог по уровню (ORE_MIN_LEVEL, shared/definitions.js) — третье
    // указание владельца: ниже него шанса нет вовсе, не уменьшенного. rlvl —
    // уровень именно ЭТОГО монстра (Room.js проставляет его при спавне), а
    // не уровень игрока: фарм зоны низкого уровня партией с сильным игроком
    // руды не даёт, как и должно быть у порога по монстру. Сотрудничество
