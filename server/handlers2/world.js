...
module.exports = function registerWorld(s, safeOn, deps) {
  ...
  function rollLoot(result) {
    ...
    //
    // Ставка одна на всех и не зависит от уровня (oreDropChance,
    // shared/definitions.js). Сотрудничество сюда не доходит вовсе — оно
    // вышло парой строк выше со своей фиксированной наградой, и это не
    // оплошность: у режима нет добычи по построению.
    if (rand() < oreDropChance()) {
