...
module.exports = function registerEconomy(s, safeOn, deps) {
  ...
  safeOn('enhanceItem', ({ id, enhance, stoneType, slot, rowId } = {}) =>
    ...
    if (res && res.outcome === 'success') {
      ...
      // ── очки сезона ────────────────────────────────────────────────────
      // Сезон 3: флэт — любой предмет, любая редкость, любой камень, пока
      // бросок успешен. Раньше (Сезон 2) очки зависели от слота/редкости/
      // типа камня; таблица снята вместе с той таблицей.
      //
      ...
      const _total = await progression.addSeasonPoints(t, pid, SEASON_ENHANCE_POINTS);
      // null означает «сезон не идёт» — тогда и говорить не о чем.
      if (_total != null) {
        s.socket.emit('seasonEventDone', { task: 'enhance', points: SEASON_ENHANCE_POINTS, total: _total });
      }
    }
    await pushAll(t);
