...
-- Schema history is a record of what happened to this database. Писать в него
-- приложение права не имеет и иметь не должно: строка о применении пишется
-- ТОЙ ЖЕ транзакцией, что и сама миграция, и право на запись позволило бы
-- объявить миграцию применённой, не применив её.
--
-- А вот ЧИТАТЬ — теперь может, и вот зачем. Пароль doadmin у нас намеренно
-- нигде не хранится (dev/migrate-now.sh спрашивает его и не пишет на диск),
-- поэтому шаг миграций в dev/server-deploy.sh без ADMIN_URL решал по файлам:
-- есть ли в выкладке миграция, которой нет в работающем дереве. После
-- честного ручного наката файл всё равно «новый» — и выкладка отказывалась бы
-- навсегда, а единственным выходом было бы положить пароль от базы с деньгами
-- в файл. Защита толкала бы ровно к тому, от чего защищает.
--
-- Спрашивать надо базу, а не файлы. Одного SELECT для этого достаточно.
--
-- И живёт это здесь, а не в миграции, по той же причине, что и REVOKE выше:
-- GRANT ... ON ALL TABLES тремя строками выше выдаётся заново на каждом
-- прогоне и перетёр бы всё, что миграция выдала раньше. Проверено — первый
-- заход сделал именно так, и грант не пережил ближайший migrate.sh.
REVOKE ALL ON schema_migrations FROM liberty_app;
GRANT SELECT ON schema_migrations TO liberty_app;
SQL
