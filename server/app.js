...
const _CLIENT_NOISE = [
  ...
  // згортають».
  //
  // «pixi жив» в условии — не украшение. У МЁРТВОГО рендерера кадр старый
  // всегда, рисовать некому, — и правило без этой части глушило
  // «world-blank:no-renderer … pixi мёртв», то есть единственный отчёт,
  // который объясняет чёрный экран. Он приходил и пропадал в счётчике.
  { where: /^(world-blank|pixi-init)$/, re: /pixi жив.*кадр \d{4,}мс назад/ },
];

// ── что не глушится НИКОГДА ────────────────────────────────────────────────
// Мёртвый рендерер — это чёрный экран у живого человека, а не свёрнутая
// вкладка. Ни одно правило выше не имеет права его закрыть, чем бы оно ни
// совпало: фильтр чистит канал ради того, чтобы в нём было видно настоящее, а
// настоящее — вот оно.
// Причина стоит в начале сообщения (js/game.js: why + ' — ' + facts), а
// «pixi мёртв» приходит из тех же фактов — совпадения по любому из них
// достаточно.
const _NEVER_NOISE = [/^no-renderer/, /^no-canvas/, /pixi мёртв/];
...
function _isClientNoise(where, message) {
  ...
  if (_NEVER_NOISE.some(r => r.test(m))) return false;
  ...
  const now = Date.now();
  // Оборванный бандл в единичном числе — сеть игрока. Десять за десять минут —
  // это уже раздача, и молчать об этом нельзя.
  if (/syntaxerror: unexpected/i.test(m) && (_noiseSeen.get(key) || 0) === 10) {
    ops.alert('client.bundle-truncated', 'Бандл доезжает не целиком',
      `10 оборванных загрузок за окно · последнее: ${m.slice(0, 120)}`).catch(() => {});
  }
...
function mountAdmin() {
  ...
  // Если сервер поднялся в режиме обслуживания — это надо сказать вслух.
  // Молчаливо закрытая игра выглядит как сломанная игра.
  if (maintenance.isOn()) {
    ops.send('alerts', '🔧 <b>Сервер поднялся В РЕЖИМЕ ОБСЛУЖИВАНИЯ</b> — '
      + 'входы отклоняются, кроме админских').catch(() => {});
  }
...
const HEAVY = new Set([
  ...
  'craftRune', 'runeSocket', 'runeUnsocket', 'runeReroll', 'runeRerollAll',
  ...
  'clanStorageUnlock', 'clanStorageSync', 'clanActivitySync',
...
for (const [channel, events] of Object.entries({
  ...
  runeCraftError: ['craftRune'],
  runeError: ['runeSocket', 'runeUnsocket', 'runeReroll', 'runeRerollAll'],
  ...
  progressError: ['learnSkill', 'upgradeSkill', 'learnForeignSkill', 'upgradeForeignSkill', 'learnPassive', 'upgradePassive',
    'learnAdvSkill', 'toggleAdvSkill'],
  questClaimError: ['claimQuest', 'completeSpecialQuest'],
  seasonError: ['seasonRating', 'seasonSync'],
  seasonBurnError: ['seasonBurnBook'],
  itemDisassembleError: ['itemDisassemble'],
  starterBonusError: ['starterBonusClaim'],
  mailBonusError: ['mailBonusClaim'],
  friendshipError: ['getFriendship', 'friendshipClaim'],
  ...
  clanError: ['clanCreate', 'clanApply', 'clanApprove', 'clanDecline', 'clanKick', 'clanLeave',
    ...
    'clanStorageDeposit', 'clanStorageDepositAll', 'clanStorageGive', 'clanStorageClaim', 'clanStorageCancel',
    'clanStorageUnlock'],
  clanStorageError: ['clanStorageSync', 'clanActivitySync'],
...
// _isClientNoise выставлен наружу ради проверки: это чистое правило о том,
// какой отчёт от клиента дойдёт до операторов, а какой нет, — и оно уже один
// раз проглотило единственное сообщение про чёрный экран (dev/blackscreen-
// check.js). Правило, которое нельзя прогнать на настоящих строках из
// журнала, проверяется чтением, а чтением его и написали.
module.exports = { app, server, io, boot, shutdown, _isClientNoise };
