...
class Room {
  ...
  updatePlayerPos(socketId, x, y, facing, moving) {
    ...
    if (p._entryGuardUntil && Date.now() < p._entryGuardUntil) {
      // ── от ТЕКУЩЕЙ позиции, а не от точки входа ──────────────────────────
      // Первый заход мерил от места постановки — и это ловило самого игрока:
      // на своих же ногах пятьсот пикселей набегаются за три секунды, а окно
      // длится восемь. Человек входил на этаж, бежал по прямой и его начинало
      // дёргать назад: «при бігу лаги, подьоргання, ривки».
      //
      // Правильная мера — расстояние от того места, где сервер видит игрока
      // СЕЙЧАС. Честный шаг это десяток пикселей за пакет, даже на крыльях;
      // пакет с прежнего этажа — сотни, потому что там другая карта. Разница
      // между ними не в скорости, а в том, что это скачок.
      const gdx = x - p.x, gdy = y - p.y;
      if (gdx * gdx + gdy * gdy > ENTRY_DRIFT_R2) {
        p._entryDrops = (p._entryDrops || 0) + 1;
        console.warn(`[Room ${this.floor}] вход: скачок на ${Math.round(Math.hypot(gdx, gdy))}px `
          + `— отказ (${p.username || socketId}, всего ${p._entryDrops})`);
