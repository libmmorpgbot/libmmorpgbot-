...
// Сколько отказов подряд предохранитель делает, прежде чем сдаться. Пять —
// это примерно одна шестая секунды при клиентских тридцати пакетах: столько
// нужно, чтобы отсечь пакеты с прежнего этажа, и мало, чтобы это заметил
// человек. Держать дольше значит спорить с клиентом, а спор на экране
// выглядит как фриз.
const ENTRY_MAX_DROPS = 5;

// Сколько тело убитого игрока остаётся в потоке. Ровно на анимацию падения:
// раньше труп не убирался вовсе и стоял на чужих экранах до перерождения.
const PLAYER_CORPSE_MS = 1500;
...
class Room {
  ...
  _tick() {
    ...
    this.enemies.forEach(e => {
      ...
      if (e.aggro && !e.trialHarmless) {
        ...
        e.atkTimer -= dt;
        if (closestD < atkReach && e.atkTimer <= 0) {
          e.atkTimer = (1.4 + Math.random() * 0.6) * (e.atkCdMult || 1);
          e.atkAnimTimer = 0.9;
          e._atkPulse = true;
  ...
  updatePlayerPos(socketId, x, y, facing, moving) {
    ...
    // ── первые пакеты после того, как игрока поставил СЕРВЕР ───────────────
    // Клиент на новом этаже ещё несколько мгновений живёт на СТАРОЙ карте —
    // она едет по сети — и всё это время шлёт прежнюю позицию. Если она
    // случайно попадает на пол новой карты, прежняя проверка (только «стена
    // или нет») её принимала и уже никогда не исправляла: игрок оказывался в
    // чужой комнате навсегда.
    //
    // ── и почему это НЕ спор с клиентом ────────────────────────────────────
    // Первый заход отказывал все восемь секунд подряд. Клиент при этом
    // продолжал бежать, серверная позиция стояла на месте — и расстояние
    // росло с каждым пакетом: 881, 888, 895, 902. Тысяча двести восемьдесят
    // шесть отказов за двенадцать часов, и каждый — рывок на экране живого
    // человека. Это и есть «біжиш і є фрізи».
    //
    // Предохранитель существует ради ПЕРВЫХ пакетов, а не ради спора. Поэтому
    // он снимается сам, как только выполнено любое из двух:
    //
    //   клиент прислал honest-пакет  значит он уже на новой карте;
    //   отказов набралось ENTRY_MAX  значит договориться не вышло, и держать
    //                                игрока дальше вреднее, чем отпустить.
    //
    // Худший случай — пять пакетов, примерно одна шестая секунды. Незаметно.
    if (p._entryGuardUntil && Date.now() < p._entryGuardUntil) {
      const gdx = x - p.x, gdy = y - p.y;
      if (gdx * gdx + gdy * gdy > ENTRY_DRIFT_R2) {
        p._entryDrops = (p._entryDrops || 0) + 1;
        if (p._entryDrops >= ENTRY_MAX_DROPS) {
          // Договориться не вышло. Отпускаем и говорим об этом один раз: если
          // это повторяется часто, дело не в клиенте.
          p._entryGuardUntil = 0;
          console.warn(`[Room ${this.floor}] вход: ${p._entryDrops} отказов подряд `
            + `(последний ${Math.round(Math.hypot(gdx, gdy))}px) — снимаю предохранитель `
            + `для ${p.username || socketId}`);
        }
        return { refused: 'entry', x: p.x, y: p.y };
      }
      // Пакет честный — клиент на новой карте, предохранитель больше не нужен.
      p._entryGuardUntil = 0;
    }
