...
async function main() {
  ...
  for (let i = 0; i < 12 && !paid; i++) {
    ...
    // Вікно між ударами скидається навмисно. Сервер приймає удар не частіше,
    // ніж раз на 1/atkSpeed (Room._attackMinGapMs) — у тестового персонажа це
    // близько 700 мс, а цикл нижче б'є одразу після відповіді. Ця перевірка про
    // ЗДОБИЧ і про те, кому вона дістається, а не про темп удару: темп має свою
    // перевірку (dev/relog-attack-check.js), і мовчазна відмова тут виглядала б
    // як «монстр не помер», хоч правило спрацювало правильно.
    me._lastAtk = 0;
