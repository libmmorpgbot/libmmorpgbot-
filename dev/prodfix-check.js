...
console.log('\n  ── регенерация HP считается сервером ──');
{
  ...
  if (typeof regen === 'function') {
    ...
    // _inSafeZone обязателен: регенерация в хабе теперь тоже серверная (см.
...
function step4() {
  ...
  console.log('\n  ── читаемость цифр ──');
  {
    ...
    // ── чем меряется читаемость ───────────────────────────────────────────
    // Здесь стояла воспринимаемая яркость с порогом 0.78, и это было верно
    // ровно до того дня, когда владелец назначил цвета сам: урон красный,
    // монеты жёлтые. У красного яркость низкая по устройству глаза (#ff5a5a —
    // 0.53), и такой порог подмешал бы в него столько белого, что «красный»
    // перестал бы им быть — то есть проверка требовала бы переписать чужое
    // решение.
    //
    // Мерой стала СВЕТЛОТА (максимальный канал) — она ловит именно то, ради
    // чего порог заводился: мутные, приглушённые цвета, у которых ни один
    // канал не дотягивает до верха. Насыщенные не трогает ни один.
    const light = (hex) => {
    ...
    // читаемым. Ниже — приглушённые, которые реально стояли в коде.
    const было = ['#8a7f6b', '#6b5f4a', '#4a5a6b', '#7a6a8a', '#5c6b4a'];
    let худший = 1;
    for (const c of было) {
      ctx.dmgNums.length = 0;
      ctx.dmgNum(0, 0, '123', c);
      худший = Math.min(худший, light(ctx.dmgNums[0].color));
    }
    ok(худший >= 0.715, 'ни один цвет не остаётся мутным', 'самый тёмный: ' + худший.toFixed(3));

    // Контроль: исходные ДОЛЖНЫ быть ниже порога, иначе проверка выше зелёная
    // просто потому, что ей нечего исправлять.
    ok(было.every(c => light(c) < 0.72),
      'контроль: исходные действительно ниже порога',
      было.map(c => `${c}=${light(c).toFixed(2)}`).join(' '));
    // И обратное: назначенные владельцем цвета проходят НЕТРОНУТЫМИ. Порог,
    // который перекрашивает красный урон, — это не страховка, а самоуправство.
    for (const c of ['#ff5a5a', '#ffd23f', '#ff8c8c']) {
      ctx.dmgNums.length = 0;
      ctx.dmgNum(0, 0, '123', c);
      ok(ctx.dmgNums[0].color === c, `${c} проходит без изменений`, ctx.dmgNums[0].color);
    }
