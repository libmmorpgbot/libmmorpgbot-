...
async function main() {
  ...
  eq(prog.empowers, 1, 'лічильник перенесено з перероджень у посилення');

  // ── запис ДО появи keptSP ────────────────────────────────────────────────
  // У таких записів перенесена через скидання рівня трата лежить усередині
  // bonusSP, а поля keptSP нема взагалі. Скопіювати як є — привезти персонажа,
  // чиї вкладені покращення не покриті ні кривою рівня, ні бонусом: анти-чит
  // зріже карту покращень при першому ж вході, і людина втратить усе, у що
  // вклалася. ETL — той єдиний момент, коли це можна полагодити.
  //
  // 30 очок у bonusSP, одне перерождення (нагорода 15) і 12 вкладених: 15
  // понад нагороду — це і є забанкована трата, і рівно вона має переїхати
  // в keptSP. Сума двох полів не змінюється.
  const legacy = await migrate({
    type: 'lev', lvl: 12, bonusSP: 30, rebirths: 1,
    upgrades: { atk: 12 },
  });
  ok(!legacy.skipped, 'старий акаунт перенесено');
  const legProg = await players.progressOf(null, legacy.playerId);
  eq(legProg.keptSP, 12, 'забанкована трата переїхала в keptSP');
  eq(legProg.bonusSP, 18, 'і рівно стільки ж пішло з bonusSP');
  eq(legProg.bonusSP + legProg.keptSP, 30, 'сума двох полів не змінилася');
