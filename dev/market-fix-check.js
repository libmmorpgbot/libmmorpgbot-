...
function clientChecks() {
  ...
  // ── заточка видна в сетке ДО клика ────────────────────────────────────────
  head('заточка видна при выборе предмета для лота — не только после клика');
  // Владелец: «при выставлении предметов при создании лота не видно заточку».
  // Две одинаковые вещи разной заточки в сетке выглядели одинаково — только
  // клик по каждой (и чтение подписи выбранного предмета) отличал их. Значок
  // — тот же, что уже стоит в инвентаре/хранилище (правый верхний угол,
  // оранжевый +N).
  ctx.player = { inventory: [
    { ...ctx.itemCatalogBase('sw1'), rowId: 1, enhance: 8, qty: 1 },
    { ...ctx.itemCatalogBase('sw1'), rowId: 2, enhance: 0, qty: 1 },
  ] };
  ctx._renderMarketPickGrid();
  const pickGridHtml = els['market-pick-grid'].innerHTML;
  ok(pickGridHtml.includes('>+8<'), 'заточенная копия показывает +8 прямо в сетке, без клика',
    pickGridHtml.slice(0, 220));
  ok((pickGridHtml.match(/>\+8</g) || []).length === 1,
    'значок стоит ровно на одной из двух ячеек — не на обеих и не потерян');
