...
PURE=(reachable gwstorage tutorial firstlang tournament skillcd bundle protocol request-shape bookpool prodfix heal skillbuff petskill teleport runes playerlog netsize death-drop spawn farmhigh farmseason craft-feedback)
...
run_one() {
  ...
  if echo "$summary" | grep -qE ', 0 (впало|упало|УПАЛО)|"ok": true|0 missing fields|запитів перевірено базою|0 без поля|"positionsRefused": 0'; then
    ...
    # ── что именно упало ────────────────────────────────────────────────────
    # Сводка говорит «1 впало» и молчит о том, ЧТО. Прогон за прогоном ронял
    # разные проверки, каждая проходила поодиночке — и разобраться было не по
    # чему: вывод исчезал вместе с переменной, а причина (чужой порт, гонка,
    # соседняя фикстура) видна только в нём.
    echo "$clean_out" | grep -E 'FAIL|✗' | head -6 | sed 's/^/              /'
    printf '%s' "$out" > "/tmp/check-${name}.log"
    printf '              полный вывод: /tmp/check-%s.log\n' "$name"
...
if [ -z "${DATABASE_URL:-}" ]; then
  ...
  # ── и убрать за собой ──────────────────────────────────────────────────────
  # Отдельной базы у проекта нет: проверки ходят по боевой и создают себе
  # игроков через players.ensure. Убирали за собой не все, и за сутки набралось
  # три тысячи аккаунтов — они попали в рейтинг (@adm-45491_victim, ур. 77,
  # 5400 БМ), в админку и в общий чат: «эти боты везде».
  #
  # Здесь, а не в каждом файле по отдельности: одно место, которое отвечает за
  # то, что после прогона база выглядит так же, как до него. Правило отбора —
  # в самом скрипте, и оно останавливается, если под него попал живой игрок.
  echo
  echo "  ── убираю за проверками ──"
  node dev/purge-test-accounts.js --apply 2>&1 | grep -E 'удалено|осталось|видны|ОСТАНОВЛЕНО|ОШИБКА' | sed 's/^/  /'
fi
