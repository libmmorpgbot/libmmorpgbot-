...
console.log('\n  ── руда ──');
{
  ...
  for (const step of ladder) {
    ...
    eq(step.chance, 1.0, `${step.from} → ${step.to}: шанс 100% — переплавка руду не съедает`);
  }
  ...
  // ── убраны счётчики и подсказки про шанс (по просьбе владельца) ──────────
  // «Убери чтоб не показывало количество и надписи про шансы тоже убери» —
  // и в сетке рецептов рун, и в лесенке переплавки. Зелёная подсветка
  // ячейки (.craftable, css/style.css) осталась — она и раньше решала «хватает
  // ли», без самих цифр.
  ok(!/\$\{have\}\/\$\{need \? need\.n : 0\}/.test(runesTab),
    'счётчик «есть/нужно» у рецепта руны убран');
  ok(!/\$\{recipe\.count\} → 1 · \$\{Math\.round\(recipe\.chance/.test(runesTab),
    'соотношение и шанс в подписи ячейки переплавки убраны');
  ok(!/\$\{have\}\/\$\{recipe\.count\}/.test(runesTab),
    'счётчик «есть/нужно» у переплавки убран');
  ok(!/Шанс успеха \$\{Math\.round\(\(RUNE_CRAFT_CHANCE/.test(runesTab),
    'подсказка про шанс ковки под сеткой рун убрана целиком');
  ok(!/Обычная руда падает с монстров/.test(runesTab),
    'подсказка про шанс дропа руды под лесенкой убрана целиком');
  const css = fs.readFileSync(path.join(ROOT, 'css/style.css'), 'utf8');
  ok(/\.craft-item-cell\.craftable/.test(css),
    'а зелёная подсветка «хватает» осталась — цифры убраны, а не сам сигнал');
...
console.log('\n  ── защиты ──');
{
  ...
  // ── ВХОД В ИГРУ НЕ РЕЖЕТ СТРОКУ ДО id/enhance/qty ───────────────────────────
  // Две жалобы владельца об одном корне.
  //
  // Первая: «руны не видны в предметах и не работают, пока не наденешь
  // что-нибудь» — session.savedView() собирала equipment/inventory/storage
  // РУЧНЫМ списком полей и молча роняла .runes.
  //
  // Вторая, найденная при разборе первой и опаснее неё: клик по гнезду руны
  // (_runeSocketTap(${it.rowId}, i), js/ui.js) при отсутствующем rowId не
  // «не находил ничего» — _findRowById сравнивает rowId БЕЗ проверки на
  // существование, и undefined===undefined совпадает с ПЕРВЫМ попавшимся
  // предметом без rowId, который может быть вообще не тем, по чему кликнули.
  // Воспроизведено на живой базе тремя предметами разом (шлем с руной,
  // оружие, сапоги в сумке) — до фикса у всех rowId===undefined; после —
  // все три различны.
  //
  // Фикс общий на все три поля: строка идёт ЦЕЛИКОМ из items.inventoryOf(),
  // а не пересобирается вручную. Проверяется тут же — что ручного списка
  // {id, enhance...} для трёх полей больше нет.
  const sesSrc = fs.readFileSync(path.join(ROOT, 'server/session.js'), 'utf8');
  const savedViewSrc = sesSrc.slice(sesSrc.indexOf('async savedView('),
    sesSrc.indexOf('async refreshClan('));
  ok(/equipment\[slot\] = it;/.test(savedViewSrc),
    'надетое идёт в save-блок целиком — rowId, rune, runes на месте, а не только id/enhance');
  ok(/inventory: inv\.inventory \|\| \[\]/.test(savedViewSrc),
    'сумка — тем же путём, не своим списком полей');
  ok(/storage: inv\.storage \|\| \[\]/.test(savedViewSrc),
    'хранилище — тем же путём');
  ok(!/id: i\.id, enhance: i\.enhance \|\| 0, qty: i\.qty \|\| 1/.test(savedViewSrc),
    'старый ручной список полей для инвентаря/хранилища убран целиком');
