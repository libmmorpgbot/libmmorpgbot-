...
console.log('\n  ── защиты ──');
{
  ...
  // ── РУНЫ НЕВИДИМЫ ПРИ ВХОДЕ ДО ПЕРВОГО EQUIP/UNEQUIP ───────────────────────
  // Жалоба владельца: «при входе в игру руны не видны в предметах и не
  // работают, пока игрок не наденет/не снимет что-нибудь». Причина —
  // session.savedView() (то, чем отвечает finishLogin) собирает equipment
  // СВОЕЙ узкой проекцией {id, enhance} и молча роняла .runes, хотя
  // fullState()→items.inventoryOf() кладёт его туда всегда. Строка написана
  // раньше рун и просто не знала о них; воспроизведено и починено на живой
  // базе (см. коммит) — здесь только заслон, чтобы не вернулось тихо.
  ...
  const savedViewSrc = sesSrc.slice(sesSrc.indexOf('async savedView('),
    sesSrc.indexOf('async savedView(') + 2000);
  ok(/equipment\[slot\]\.runes = it\.runes/.test(savedViewSrc),
    'вход в игру несёт .runes на надетых вещах, а не только id/enhance');
